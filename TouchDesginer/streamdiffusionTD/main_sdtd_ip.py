        ...
        try:
            ...
            if mode == "img2img":
                ...
                if output_memory is None:
                    output_memory = shared_memory.SharedMemory(name=output_mem_name, create=True, size=output_images_np.nbytes)
                output_array = np.ndarray(output_images_np.shape, dtype=output_images_np.dtype, buffer=output_memory.buf)
                output_array[:] = output_images_np[:]
